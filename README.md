<div align="center">

# 🚀 快速解锁被阻止的应用

### 通过 Alfred 快速解除 macOS 应用的安全拦截

[![GitHub release](https://img.shields.io/github/release/dontloseyourway/alfred-allow-blocked-apps.svg)](https://github.com/dontloseyourway/alfred-allow-blocked-apps/releases)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
[![macOS](https://img.shields.io/badge/macOS-10.15+-blue.svg)](https://www.apple.com/macos/)
[![Alfred](https://img.shields.io/badge/Alfred-4.0+-purple.svg)](https://www.alfredapp.com/)

**告别繁琐的系统设置操作!**

[功能特性](#-功能特性) • [安装说明](#-安装说明) • [使用方法](#-使用方法) • [开发指南](#️-开发指南) • [参与贡献](#-参与贡献)

![Demo](https://via.placeholder.com/800x400.png?text=Demo+Screenshot)

</div>

---

## ✨ 功能特性

- 🎯 **Alfred 集成** - 直接在 Alfred 中操作,无需打开系统设置
- 📱 **可视化图标** - 显示应用图标,方便识别  
- ⚡ **极速解锁** - 3秒完成 vs 手动操作需要30-60秒
- 🔍 **自动扫描** - 自动发现所有被拦截的应用
- 💫 **批量操作** - 一键解锁所有被阻止的应用
- ✅ **系统通知** - 即时反馈操作结果

---

## 📦 安装说明

### 方法一: 下载发布版本(推荐)

1. 从 [Releases](https://github.com/dontloseyourway/alfred-allow-blocked-apps/releases) 下载最新的 `Allow-Blocked-Apps.alfredworkflow`
2. 双击文件进行安装
3. Alfred 会自动导入该工作流

### 方法二: 从源码构建

```bash
# 克隆仓库
git clone https://github.com/dontloseyourway/alfred-allow-blocked-apps.git
cd alfred-allow-blocked-apps

# 构建工作流
python3 build.py

# 安装
open Allow-Blocked-Apps.alfredworkflow
```

### 系统要求

- macOS 10.15 (Catalina) 或更高版本
- [Alfred 4](https://www.alfredapp.com/) 或更高版本(需要 Powerpack)
- Python 3 (仅从源码构建时需要)

---

## 🎯 使用方法

### 基础用法

1. 按下 Alfred 快捷键(默认: `⌥ + Space`)
2. 输入 `allow`
3. 从列表中选择被拦截的应用
4. 按 `Enter` 确认
5. 完成! 应用已解锁

### 批量解锁

选择第一项 **"🎯 解锁所有被阻止的应用"** 即可一次性解锁所有应用。

### 操作演示

```
在 Alfred 中输入 "allow":

┌─────────────────────────────────────────┐
│ 🎯 解锁所有被阻止的应用 (5个应用)       │
│    一次性解锁所有应用                   │
├─────────────────────────────────────────┤
│ 🚫 Alfred 5                     [图标]  │
│    点击解锁此应用                       │
├─────────────────────────────────────────┤
│ 🚫 Figma                        [图标]  │
│    点击解锁此应用                       │
└─────────────────────────────────────────┘

选择后 → 通知: "✅ 应用已解锁"
```

---

## ⚡ 性能对比

| 方法 | 步骤数 | 耗时 | 是否打断工作流 |
|--------|-------|------|----------------------|
| **手动(系统设置)** | 6步 | 30-60秒 | 是 |
| **本工作流** | 2步 | 3秒 | 否 |

**快10倍!** ⚡

---

## 🛠️ 开发指南

### 项目结构

```
alfred-allow-blocked-apps/
├── src/
│   ├── alfred_list.sh              # 扫描被拦截的应用
│   └── alfred_allow.sh             # 移除隔离属性
├── build.py                        # 构建脚本
├── Allow-Blocked-Apps.alfredworkflow
├── README.md
├── LICENSE
├── CHANGELOG.md
├── CONTRIBUTING.md
└── .gitignore
```

### 构建方法

```bash
# 修改 src/ 目录中的脚本
vim src/alfred_list.sh

# 重新构建工作流
python3 build.py

# 测试
open Allow-Blocked-Apps.alfredworkflow
```

### 工作原理

macOS Gatekeeper 会为下载的应用添加 `com.apple.quarantine` 扩展属性。本工作流使用以下命令移除该属性:

```bash
sudo xattr -rd com.apple.quarantine /Applications/YourApp.app
```

### 脚本说明

#### `src/alfred_list.sh`
- 扫描 `/Applications` 目录
- 查找带有隔离属性的应用  
- 输出 Alfred Script Filter JSON 格式
- 包含应用图标

#### `src/alfred_allow.sh`
- 接受应用路径或 "ALL" 作为参数
- 使用 `xattr` 移除隔离属性
- 通过 `osascript` 发送系统通知
- 需要 sudo 权限

---

## 🔧 高级配置

### 自定义快捷键

1. 打开 Alfred 偏好设置
2. 导航到该工作流
3. 添加快捷键触发器(例如: `⌘ + ⌥ + A`)
4. 连接到 Script Filter

### 自定义关键词

在 Script Filter 设置中将关键词从 "allow" 改为你喜欢的任何词。

---

## ❓ 常见问题

**问: 需要 Alfred Powerpack 吗?**  
答: 是的,工作流功能需要 Alfred Powerpack(付费版本)。

**问: 这安全吗?**  
答: 完全安全。它使用苹果官方的 `xattr` 命令。代码开源可供审查。

**问: 为什么需要 sudo?**  
答: 移除扩展属性需要管理员权限。你只需输入一次密码。

**问: 支持哪些应用?**  
答: 所有位于 `/Applications` 中被 Gatekeeper 拦截的 `.app` 应用包。

**问: 可以撤销吗?**  
答: 一旦解锁,应用就能正常运行。无法通过本工具重新拦截(需使用系统设置)。

---

## 🤝 参与贡献

欢迎贡献! 请查看 [CONTRIBUTING.md](CONTRIBUTING.md) 了解贡献指南。

### 贡献方式

- 🐛 报告问题
- 💡 提出功能建议
- 📝 改进文档
- 🔧 提交 Pull Request

---

## 📝 更新日志

查看 [CHANGELOG.md](CHANGELOG.md) 了解版本历史。

---

## 📄 开源协议

本项目采用 MIT 协议 - 详见 [LICENSE](LICENSE) 文件。

---

## 🙏 致谢

- 感谢 Alfred 社区的灵感
- 用 ❤️ 为 macOS 用户打造

---

## 🌟 Star 历史

如果觉得这个项目有用,请考虑给个 Star ⭐

---

<div align="center">

**[⬆ 返回顶部](#-快速解锁被阻止的应用)**

由 [贡献者](https://github.com/dontloseyourway/alfred-allow-blocked-apps/graphs/contributors) 用 ❤️ 制作

</div>
